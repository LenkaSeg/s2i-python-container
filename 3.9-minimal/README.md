...
The full container image is an universal base image to build your containerized applications on top of it. However, its universal nature
...
of installed packages. There are no compilers, no header files, no npm etc and dnf tool is replaced with minimalistic
reimplementation called microdnf making the resulting container images much smaller. It also causes some limitations
but we also provide ways how to workaround them.
